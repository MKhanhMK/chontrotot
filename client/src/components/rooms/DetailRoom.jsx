...
const DetailRoom = ({ room }) => {
  ...
  useEffect(() => {
    if (room) {
      reset({
        ...
        garbagePrice: room?.garbagePrice,
  ...
  return (
    <div
      ...
      className="w-[90%] md:w-[80%] pb-[100px] lg:w-[550px] max-h-[90vh] overflow-y-auto bg-white rounded-md"
    >
        ...
        <table className="max-w-full w-full overflow-x-auto">
          <thead>
            <tr>
              ...
              <th className="border p-3 text-center">Rác</th>
                      ...
                      onClick={() =>
                        handleUpdatePayment(
                          ...
                          el.electric * room?.electricPrice +
                            ...
                            (el.garbage ? 1 : 0) * room?.garbagePrice +
      ...
      <div className="p-5 w-full">
        <h3 className="font-semibold mb-5">Lịch sử thanh toán</h3>
        <table className="max-w-full w-full overflow-x-auto">
          <thead>
            <tr>
              <th className="border p-4 text-center">Ngày thanh toán</th>
              <th className="border p-4 text-center">Số điện thoại</th>
              <th className="border p-4 text-center">Số tiền</th>
          ...
          <tbody>
            {room?.rPayment?.map((el) => (
              <tr key={el.id}>
                <td className="border p-4 text-center">{moment(el.createdAt).format("DD/MM/YY")}</td>
               
                <td className="border p-4 text-center">{el.rUser?.phone}</td>
                <td className="border p-4 text-center">{formatMoney(el.total)}</td>
                <td className="border p-4 text-center">{el.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
