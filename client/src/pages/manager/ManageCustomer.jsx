          ...
          <table className="max-w-full w-full">
            <thead>
              <tr>
                ...
                <th className="border p-3 hidden md:table-cell text-center">
                  Giới tính
                </th>
                <th className="border p-3 hidden lg:table-cell text-center">
                  Email
                </th>
                <th className="border hidden md:table-cell p-3 text-center">
                  Năm sinh
                </th>
                ...
                <tr key={el.id}>
                  ...
                  <td className="border p-3 text-center">
                    <span className="line-clamp-2">
                      {el.rprofile?.lastName + " " + el.rprofile?.firstName}
                    </span>
                  </td>
                  ...
                  <td className="border p-3 text-center">
                    <span className="flex items-center justify-center">
                      <img
                        src={el.rprofile?.image || "/user.svg"}
                        alt=""
                  ...
                  <td className="border p-3 text-center">
                    {el.rprofile?.address.length > 20
                      ? el.rprofile?.address.slice(0, 20) + "..."
                      : el.rprofile?.address}
                  </td>
                  <td className="border p-3 hidden md:table-cell text-center">
                    {el.rprofile?.gender}
                  </td>
                  <td className="border p-3 hidden lg:table-cell text-center">
                    {el.rprofile?.email || "Chưa cập nhật"}
                  </td>
                  <td className="border hidden md:table-cell p-3 text-center">
                    {moment(el.birthday).format("YYYY")}
                  </td>
                  <td className="border p-3 text-center">{el.rprofile?.CID}</td>
                  <td className="border p-3 text-center">
